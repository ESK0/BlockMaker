...
public void Func_CreateBlock(int client, const char[] szType)
{
  ...
  if(DispatchSpawn(iBlock))
  {
    ...
    fOrigin[2] += 20.0;
    //Func_CreateParticle(iBlock, "teleport01");
    ...
    {
      SDKHook(iBlock, SDKHook_StartTouch, EventSDK_OnStartTouch_Trampoline);
      arBlockMakerData[iIndex].PushString("100");
      SetEntPropString(iBlock, Prop_Data, "m_iGlobalname", "100");
    }
  }
}
...
public void Func_StringToVector(char[] szSource, float fVector[3])
{
...
public void Func_VectorToString(float fVector[3], char[] szOutput, int len)
{
  Format(szOutput, len, "%f;%f;%f", fVector[0], fVector[1], fVector[2]);
}
public bool Func_IsVectorZero(float fl[3])
{
...
public void Func_GetBlockType(int iBlock, char[] szBlockType, int len)
{
  if(IsValidEntity(iBlock))
  {
    char szBuffer[32];
...
public void Func_GetBlockSize(int iBlock, char[] szBlockSize, int len)
{
  if(IsValidEntity(iBlock))
  {
    char szBuffer[32];
...
public int Func_GetAimOrigin(int client, float hOrigin[3])
{
  float vAngles[3];
  float fOrigin[3];
...
public void Func_PrecacheAndDownloadModels()
{
  ...
  while(hDownloadFile.ReadLine(sDownloadFile, sizeof(sDownloadFile)))
  {
    ...
    if(FileExists(sDownloadFile) == true)
    {
      ...
      /*if(StrContains(sDownloadFile,".pcf",false) == iNamelen)
      {
        PrecacheGeneric(sDownloadFile, true);
      }*/
